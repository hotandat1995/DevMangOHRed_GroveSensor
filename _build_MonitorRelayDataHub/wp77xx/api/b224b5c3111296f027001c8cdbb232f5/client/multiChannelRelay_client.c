...
#include "multiChannelRelay_interface.h"
...
typedef struct
{
    ...
    multiChannelRelay_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(multiChannelRelay_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_multiChannelRelay_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_multiChannelRelay_HasLocalBinding())
    {
        return NULL;
    }
    else
    {
        ...
        LE_FATAL_IF(clientThreadPtr==NULL,
                    "multiChannelRelay_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    ifgen_multiChannelRelay_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(multiChannelRelay_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
 ...
 */
//--------------------------------------------------------------------------------------------------
void multiChannelRelay_ConnectService
(
    void
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t multiChannelRelay_TryConnectService
(
    void
)
{
...
)
{
    ...
    LE_FATAL("Component for multiChannelRelay disconnected\n");
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void multiChannelRelay_SetServerDisconnectHandler
(
    multiChannelRelay_DisconnectHandler_t disconnectHandler,
    void *contextPtr
)
{
    if (ifgen_multiChannelRelay_HasLocalBinding())
 ...
 */
//--------------------------------------------------------------------------------------------------
void multiChannelRelay_DisconnectService
(
    void
)
{
...
{
     ifgen_multiChannelRelay_turn_off_channel(
...
{
     ifgen_multiChannelRelay_turn_on_channel(
        GetCurrentSessionRef(),
        channel
    );
}
...
{
    return ifgen_multiChannelRelay_getChannelState(
        GetCurrentSessionRef()
    );
}