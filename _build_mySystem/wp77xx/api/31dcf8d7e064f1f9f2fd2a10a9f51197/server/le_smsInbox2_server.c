 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_smsInbox2_ServerData, HIGH_SERVER_DATA_COUNT, sizeof(_ServerData_t));
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_smsInbox2_ServerHandlers, HIGH_SERVER_DATA_COUNT);
...
)
{
    ...
    _HandlerRefMap = le_ref_InitStaticMap(le_smsInbox2_ServerHandlers, HIGH_SERVER_DATA_COUNT);
...
)
{
    ...
    if ( (pduSize > 176) )
    {
        LE_DEBUG("Adjusting pduSize from %" PRIuS " to 176", pduSize);
        pduSize = 176;
    }

    // Define storage for output parameters
    uint8_t pduBuffer[176] = { 0 };
