...
#include "multiChannelRelay_server.h"
#include "multiChannelRelay_messages.h"
#include "multiChannelRelay_service.h"
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(multiChannelRelay_ServerData, HIGH_SERVER_DATA_COUNT, sizeof(_ServerData_t));
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(multiChannelRelay_ServerHandlers, HIGH_SERVER_DATA_COUNT);
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(multiChannelRelay_ServerData,
    ...
    _HandlerRefMap = le_ref_InitStaticMap(multiChannelRelay_ServerHandlers, HIGH_SERVER_DATA_COUNT);
...
static void Handle_multiChannelRelay_turn_off_channel
(
...
)
{
    ...
    // Call the function
    multiChannelRelay_turn_off_channel ( 
        channel );
...
static void Handle_multiChannelRelay_turn_on_channel
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    multiChannelRelay_turn_on_channel ( 
        channel );
...
static void Handle_multiChannelRelay_getChannelState
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    uint8_t _result;
    _result  = multiChannelRelay_getChannelState (  );

    // Re-use the message buffer for the response
    _msgBufPtr = _msgBufStartPtr;

    // Pack the result first
    LE_ASSERT(le_pack_PackUint8( &_msgBufPtr, _result ));
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_multiChannelRelay_turn_off_channel :
            Handle_multiChannelRelay_turn_off_channel(msgRef);
            break;
        case _MSGID_multiChannelRelay_turn_on_channel :
            Handle_multiChannelRelay_turn_on_channel(msgRef);
            break;
        case _MSGID_multiChannelRelay_getChannelState :
            Handle_multiChannelRelay_getChannelState(msgRef);
            break;
